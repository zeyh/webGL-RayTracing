...
VBO_genetic.prototype.setMaterial = function (code) {
    this.g_matl0.setMatl(code);
};
VBO_genetic.prototype.init = function () {
    ...
    // ! init VBO
    // * [Vertex] buffer
    this.vertexBuffer = initArrayBufferForLaterUse(
        gl,
        this.vertices,
        3,
        gl.FLOAT
    ...
    if (!this.vertexBuffer) {
        console.log(
            this.constructor.name +
                ".init() failed to create VBO in GPU. Bye! [vertex buffer]"
        );
        return;
    }
    // * [Index] buffer
    if (this.indices.length > 0) {
        this.indexBuffer = initElementArrayBufferForLaterUse(
            gl,
            this.indices,
            gl.UNSIGNED_BYTE
        );
        if (!this.indexBuffer) {
            console.log(
                this.constructor.name +
                    ".init() for [indices] failed to create VBO in GPU. Bye!"
            );
            return;
        }
    }
    // * [Color] buffer
    if (this.lightSpec != 2 && this.lightSpec != 3 && this.lightSpec != 5) {
        //assign color
        this.colorBuffer = initArrayBufferForLaterUse(
            gl,
            this.colors,
            3,
            gl.FLOAT
        ...
        if (this.a_ColrLoc < 0 || !this.colorBuffer) {
            console.log(
                this.constructor.name +
                    ".init() failed to create VBO in GPU. Bye! [colorBuffer]"
            );
            return;
        }
    }
    // * [Normal] Buffer
    if (this.lightSpec != 4) {
        this.normalBuffer = initArrayBufferForLaterUse(
            gl,
            this.normals,
            3,
            gl.FLOAT
        ...
        if (!this.normalBuffer) {
            console.log(
                this.constructor.name +
                    ".init() failed to create VBO in GPU. Bye! [normalBuffer]"
            );
            return;
        }
        ...
        this.u_NormMatLoc = gl.getUniformLocation(
            this.shaderLoc,
            "u_NormalMatrix"
        );
    ...
    if (this.lightSpec != 0 && this.lightSpec != 1) {
        this.u_eyePosWorld = gl.getUniformLocation(
            this.shaderLoc,
            "u_eyePosWorld"
        );
        if (!this.u_eyePosWorld) {
            console.log(
                "Failed to get matrix storage locations [u_eyePosWorld]"
            );
            return;
        }
    }
    ...
    if (this.lightSpec == 1) {
        ...
        this.u_LightColor = gl.getUniformLocation(
            this.shaderLoc,
            "u_LightColor"
        );
    ...
    if (this.lightSpec == 2) {
        ...
        if (!this.u_Ke || !this.u_Ka || !this.u_Kd || !this.u_Ks) {
            console.log(
                "Failed to get the Phong Reflectance storage locations"
            );
            return;
        }
    }
    ...
    if (this.lightSpec == 3 || this.lightSpec == 5) {
        ...
        this.g_lamp0.u_pos = gl.getUniformLocation(
            this.shaderLoc,
            "u_LampSet[0].pos"
        );
        this.g_lamp0.u_ambi = gl.getUniformLocation(
            this.shaderLoc,
            "u_LampSet[0].ambi"
        );
        this.g_lamp0.u_diff = gl.getUniformLocation(
            this.shaderLoc,
            "u_LampSet[0].diff"
        );
        this.g_lamp0.u_spec = gl.getUniformLocation(
            this.shaderLoc,
            "u_LampSet[0].spec"
        );
        if (
        ...
        this.g_matl0.uLoc_Ke = gl.getUniformLocation(
            this.shaderLoc,
            "u_MatlSet[0].emit"
        );
        this.g_matl0.uLoc_Ka = gl.getUniformLocation(
            this.shaderLoc,
            "u_MatlSet[0].ambi"
        );
        this.g_matl0.uLoc_Kd = gl.getUniformLocation(
            this.shaderLoc,
            "u_MatlSet[0].diff"
        );
        this.g_matl0.uLoc_Ks = gl.getUniformLocation(
            this.shaderLoc,
            "u_MatlSet[0].spec"
        );
        this.g_matl0.uLoc_Kshiny = gl.getUniformLocation(
            this.shaderLoc,
            "u_MatlSet[0].shiny"
        );
        if (
            !this.g_matl0.uLoc_Ke ||
            !this.g_matl0.uLoc_Ka ||
            !this.g_matl0.uLoc_Kd ||
            !this.g_matl0.uLoc_Ks ||
            !this.g_matl0.uLoc_Kshiny
        ) {
            console.log(
                "Failed to get the Phong Reflectance storage locations"
            );
    ...
    // * [Second] Lamp
    if (this.lightSpec == 3) {
        this.g_lamp1.u_pos = gl.getUniformLocation(
            this.shaderLoc,
            "u_LampSet[1].pos"
        );
        this.g_lamp1.u_ambi = gl.getUniformLocation(
            this.shaderLoc,
            "u_LampSet[1].ambi"
        );
        this.g_lamp1.u_diff = gl.getUniformLocation(
            this.shaderLoc,
            "u_LampSet[1].diff"
        );
        this.g_lamp1.u_spec = gl.getUniformLocation(
            this.shaderLoc,
            "u_LampSet[1].spec"
        );
        if (
    ...
    if (this.lightSpec == 4) {
        ...
        if (!this.u_FogColor || !this.u_FogDist) {
            console.log(
                "Failed to get the storage location [FOG color && dist]"
            );
            return;
        }
    }
...
VBO_genetic.prototype.switchToMe = function () {
    //similar to previous set-up for draw()
    ...
    if (this.lightSpec == 3) {
        ...
        gl.uniform1i(
            this.g_matl0.uLoc_Kshiny,
            parseInt(this.g_matl0.K_shiny, 10)
        );
    }
    if (this.lightSpec == 5) {
        // * [eye] position world
        var eyePosWorld = new Float32Array(3); // x,y,z in world coords
        eyePosWorld.set([gui.camEyePt[0], gui.camEyePt[1], gui.camEyePt[2]]);
        gl.uniform3fv(this.u_eyePosWorld, eyePosWorld);
...
VBO_genetic.prototype.isReady = function () {
    //very brief sanity check
    var isOK = true;
    ...
    if (
        ...
        gl.getParameter(gl.ELEMENT_ARRAY_BUFFER_BINDING) != this.indexBuffer
    ) {
        console.log(
            this.constructor.name +
...
VBO_genetic.prototype.draw = function (
    g_modelMatrix,
    g_viewProjMatrix,
    isGrid
) {
