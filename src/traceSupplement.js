/** 
 * Copied from Week01_traceSupplement_READY.js
 */
...
CGeom.prototype.traceGrid = function (inRay) {
    ...
    var hitPt = vec4.fromValues(
        inRay.orig[0] + inRay.dir[0] * t0,
...
CImgBuf.prototype.setTestPattern = function (pattNum) {
    ...
    if (pattNum < 0 || pattNum >= PATT_MAX) pattNum %= PATT_MAX; // prevent out-of-range inputs.
    //console.log('pattNum: ', pattNum);

    // use local vars to set the array's contents.
    for (var j = 0; j < this.ySiz; j++) {
        // for the j-th row of pixels
        for (var i = 0; i < this.xSiz; i++) {
            //  & the i-th pixel on that row,
...
CImgBuf.prototype.int2float = function () {
    ...
    for (var j = 0; j < this.ySiz; j++) {
        // for each scanline
        for (var i = 0; i < this.xSiz; i++) {
            // for each pixel on that scanline
